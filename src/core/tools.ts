...
export function registerTools(server: FastMCP) {
  ...
  server.addTool({
    ...
    parameters: z.object({
      ...
      name: z.enum(services.SHADCN_VUE_COMPONENTS).describe("name of the component from shadcn/vue, lowercase, kebab-case"),
    }),
    execute: async (params) => {
      try {
        ...
        const demos = await services.ComponentServices.fetchUsageDemo(params.name);
        
        // 将文档中的 <ComponentPreview name="组件名" /> 替换为对应的 demo 代码
        // 确保demos是数组类型
        const demosArray = Array.isArray(demos) ? demos : [];
        const processedDoc = services.ComponentServices.replaceComponentPreviewsWithCode(doc, demosArray);
    ...
    execute: async (params) => {
      try {
        ...
        const filteringPrompt = `
            ...
            ${services.SHADCN_VUE_CHARTS.map((chart) => getInfo("charts", chart)).join(
              "\n"
            )}
            </available-components>
        `;
